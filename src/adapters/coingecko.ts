...
async function priceById(id: string, fiat: Fiat): Promise<number | undefined> {
...
async function priceByContract(contract: string, fiat: Fiat): Promise<number | undefined> {
  const url = `${base}/simple/token_price/ethereum?contract_addresses=${contract}&vs_currencies=${fiat}`;
  const r = await fetch(url, { headers: { accept: 'application/json' } });
  if (!r.ok) return;
  const j = await r.json();
  const item = j?.[contract.toLowerCase()];
  return item?.[fiat];
}
